...
def extra_data(date):  # 提取数据
    ...
    df_basic["list_date"] = df_basic["list_date"].astype(str)
    df_basic["delist_date"] = df_basic["delist_date"].astype(str)
    # df_basic = pro.opt_basic(exchange='SSE', fields='ts_code,name,call_put,exercise_price,list_date,delist_date')
    df_basic = df_basic[
        (df_basic.list_date <= date) & (df_basic.delist_date > date)
    ]  # 提取当天市场上交易的期权合约
...
def data_clear(df):  # 数据清洗
    ...
    df_p = df_p.rename(
        columns={"c": "c_p", "ts_code": "ts_code_p", "call_put": "call_put_p"}
    )
    ...
    df_p = df_p.rename(
        columns={"c_p": "c", "ts_code_p": "ts_code", "call_put_p": "call_put"}
    )
...
def bsm_value(s, k, t, r, sigma, q, option_type):
    ...
    if option_type.lower() == "c":
        value = s * exp(-q * t) * stats.norm.cdf(d1) - k * exp(-r * t) * stats.norm.cdf(
            d2
        )
    else:
        value = k * exp(-r * t) * stats.norm.cdf(-d2) - s * exp(
            -q * t
        ) * stats.norm.cdf(-d1)
    return value
...
def im_surface(df):  # 波动率曲面作图
    ...
    fig.update_layout(
        scene=dict(xaxis_title="剩余期限", yaxis_title="执行价格", zaxis_title="隐含波动率"),
        width=1400,
...
def main():
    loginresult = c.start("LoginMode=SXDL,PhoneNumber=13072181683")
    print(loginresult)
